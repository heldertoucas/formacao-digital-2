...
import { Text, Group, Stack, Container, Title, SimpleGrid, ThemeIcon, Box } from "@mantine/core";
...
const TestimonialCard = ({ testimonial }) => {
  return (
    <Card shadow="lg" radius="md" padding="xl">
      <Stack align="center" gap="md">
        <ThemeIcon size={50} radius="xl">
          <IconQuote style={{ width: '60%', height: '60%' }} />
        </ThemeIcon>
        <Text ta="center" fz="lg" style={{ fontStyle: 'italic' }}>
          "{testimonial.quote}"
        </Text>
        <Stack align="center" gap={0}>
          <Text fw={700}>{testimonial.author}</Text>
          <Text c="dimmed">{testimonial.role}</Text>
        </Stack>
      </Stack>
    </Card>
  );
...
export function TestimonialsSection() {
  return (
    <Box component="section" id="testimonials" py={{ base: 'xl', md: '5rem' }}>
        <Container size="xl">
            <Stack align="center" mb="xl">
                <Title order={2} fz={{ base: '2rem', md: '3rem' }} ta="center">
